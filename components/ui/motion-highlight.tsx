...
export const title = "Motion Highlight";
...
function MotionHighlight<T extends string>({
  ...
  const [highlightStyleState, setHighlightStyleState] =
    React.useState<React.CSSProperties>(highlightStyle || {});
  ...
  const setActiveClassName = React.useCallback((className: string) => {
    setActiveClassNameState(className);
  }, []);
            ...
            <AnimatePresence initial={false}>
              {boundsState && (
                <motion.div
                  ...
                  transition={transition}
                  className={cn(
                    "absolute rounded-lg z-0 backdrop-blur-sm",
                    className,
                    activeClassNameState
                  )}
                  style={highlightStyleState}
                />
              )}
            </AnimatePresence>
            {children}
    ...
    <MotionHighlightContext.Provider
      value={{
        ...
        forceUpdateBounds: (props as ParentModeMotionHighlightProps)
          ?.forceUpdateBounds,
        highlightStyle: highlightStyleState,
      }}
    >
      {enabled
        ? controlledItems
          ...
          <AnimatePresence initial={false}>
            {isActive && !isDisabled && (
              <motion.div
                ...
                data-slot="motion-highlight"
                className={cn(
                  "absolute inset-0 rounded-lg z-0 backdrop-blur-sm",
                  contextClassName,
                  activeClassName
                )}
