...
export function InventoryGrid({ items, onEdit, onDelete, loading }: InventoryGridProps) {
  if (loading) {
      ...
      {items.map((item) => (
        <Card key={item.item_id} className="flex flex-col">
          ...
          <CardContent className="flex-1">
            ...
            <p className="text-sm text-gray-600">Safety Stock: {item.safetyStock ?? 0}</p>
            {item.location && <p className="text-sm text-gray-600">Location: {item.location}</p>}
            {item.description && (
              <p className="text-sm text-gray-600 truncate">Desc: {item.description}</p>
            )}
            <p className="text-sm text-gray-600">Cost: ${item.unitCost.toFixed(2)}</p>
          </CardContent>
