...
interface SquaresProps {
  ...
  lineWidth?: number; // New prop for line thickness
...
const Squares: React.FC<SquaresProps> = ({
  direction = "right",
  speed = 1,
  borderColor = "rgba(255, 255, 255, 1)", // White with 60% opacity
  squareSize = 60,
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // Apply opacity to the border color if it's a string
      ...
      if (typeof borderColor === "string" && borderColor.startsWith("#")) {
        ...
        effectiveBorderColor = `rgba(${r}, ${g}, ${b}, ${opacity})`;
      }

      const startX = Math.floor(gridOffset.current.x / squareSize) * squareSize;
      const startY = Math.floor(gridOffset.current.y / squareSize) * squareSize;

      // Set line properties for thinner lines
